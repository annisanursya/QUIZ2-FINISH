...
public class StudentFormActivity extends AppCompatActivity {
     ...
     * 1. NIM must be all numbers and 10 digits
     * 2. Name must not be empty
     * 3. Email based validation
     * 4. Phone must be number with no less than 9 numbers
     * 4. Any other field are optionals
     *
     * @param student
    ...
    private boolean validateStudent(Student student) {
        ...
        if (student.getNoreg().length() != 10) {
            nameText.setError("Noreg must containts 10 characters");
            isValidated = false;
        } else if {
            Pattern NoregPattern = Pattern.compile("[0-9]{10}");
            nameText.setError("Noreg must all numbers");
            isValidated = false;
        }

        if (student.getPhone().length() > 9) {
            nameText.setError("phone must be number with no less than 9 numbers");
            isValidated = false;
        } else if {
            Pattern NoregPattern = Pattern.compile("[0-9]{10}");
            nameText.setError("phone must be number with no less than 9 numbers");
    ...
    private boolean isValidMail(String email2) {
        ...
        if (!check) {
            mailText.setError("Not Valid Email");
        }
        return check;
    }
}
